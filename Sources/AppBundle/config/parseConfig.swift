...
private let configParser: [String: any ParserProtocol<Config>] = [
    ...
    "scrolling-focus-alignment": Parser(\.scrollingFocusAlignment, parseScrollFocusAlignment),
    "scrolling-column-width": Parser(\.scrollingColumnWidth) { raw, backtrace in
        parseInt(raw, backtrace).filter(.semantic(backtrace, "Must be in [30, 100] range")) { (30 ... 100).contains($0) }
    },
...
private func parseScrollFocusAlignment(_ raw: Json, _ backtrace: ConfigBacktrace) -> ParsedConfig<ScrollFocusAlignment> {
    parseString(raw, backtrace).flatMap {
        ScrollFocusAlignment(rawValue: $0)
            .orFailure(.semantic(backtrace, "Can't parse scroll focus alignment '\($0)'"))
    }
}
...
func parseMonitorDescription(_ raw: Json, _ backtrace: ConfigBacktrace) -> ParsedConfig<MonitorDescription> {
    parseString(raw, backtrace).flatMap {
        Common.parseMonitorDescription($0).mapError { .semantic(backtrace, $0) }
    }
}
