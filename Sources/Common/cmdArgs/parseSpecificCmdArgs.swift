func parseSpecificCmdArgs<T: CmdArgs>(_ raw: T, _ args: StrArrSlice) -> ParsedCmd<T> {
    ...
    while index < args.count {
        ...
        } else if arg.starts(with: "-") && !isSetColumnWidthNegativePercentArg(raw, arg: arg) {
...
private func isSetColumnWidthNegativePercentArg(_ raw: any CmdArgs, arg: String) -> Bool {
    var iter = arg.makeIterator()
    return raw is SetColumnWidthCmdArgs && iter.next() == "-" && iter.next()?.isNumber == true && arg.hasSuffix("%")
}
