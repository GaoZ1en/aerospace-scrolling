...
* Change tiling layout but preserve orientation: `tiles|accordion|scrolling`
...
* Toggle between `tiles`, `accordion`, and `scrolling` layouts (order of args doesn't matter): +
`aerospace layout tiles accordion scrolling`
...
* Switch the workspace root to the scrolling column layout: +
