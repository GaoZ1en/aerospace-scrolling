...
. <<layouts,Layout>> (Possible values: `tiles`, `accordion`, `scrolling`)
...
Scrolling is a layout where root-level containers are arranged as horizontal columns.
The focused column is centered in the visible workspace.
You can use xref:commands.adoc#scroll[scroll], xref:commands.adoc#center-column[center-column],
xref:commands.adoc#move-column[move-column], and xref:commands.adoc#set-column-width[set-column-width]
to navigate and adjust the scrolling columns.
